...
def residuals_totalcases(list_all_params):
    ...
    for k, (continent_k, country_k, province_k) in enumerate(list_tuples_with_data):
        ...
        residuals_value_i = sum(
            np.multiply((x_sol_i[15, :] - fitcasesnd_i) ** 2, weights_i)
            + balance_i * balance_i * np.multiply((x_sol_i[14, :] - fitcasesd_i) ** 2, weights_i)
        )
        residuals_value_total += residuals_value_i
