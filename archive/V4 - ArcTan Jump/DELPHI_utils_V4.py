...
class DELPHIDataCreator:
    def __init__(
        ...
        if testing_data_included:
            assert len(best_params) == 15,  f"Expected 9 best parameters, got {len(best_params)}"
        else:
            assert len(best_params) == 12, f"Expected 7 best parameters, got {len(best_params)}"
        ...
        df_parameters = pd.DataFrame({
            ...
            "Internal Parameter 1": [self.best_params[6]], "Internal Parameter 2": [self.best_params[7]]  ,
                        "Jump Magnitude": [self.best_params[8]], "Jump Time": [self.best_params[9]] ,
                                    "Median Day of Action 2": [self.best_params[10]], "Rate of Action": [self.best_params[11]]
        })
