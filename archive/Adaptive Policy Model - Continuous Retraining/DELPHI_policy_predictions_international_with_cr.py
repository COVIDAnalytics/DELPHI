...
for continent, country, province in zip(
    ...
    if (
        ...
        if len(df_temp_policy_change_tracking_tuple_previous) == 0:
            print("Should create the country/province's line in main fitting script and then come back to this one"
                  " for pure policy predictions (this script doesn't optimize)")
            continue
        ...
        if (
            ...
            print(f"Don't have any data in policy tracking on {yesterday} for ({country}, {province}), "
                  f"should fit 3.0+CR on that date then run this script")
            continue
        elif (
                POLICY_TRACKING_ALREADY_EXISTS and
                (df_temp_policy_change_tracking_tuple_previous.date.max() >= str(pd.to_datetime(yesterday).date()))
            ...
            # We only take the row corresponding to date "yesterday"
            df_temp_policy_change_tracking_tuple_updated = df_temp_policy_change_tracking_tuple_updated.iloc[
                    (df_temp_policy_change_tracking_tuple_updated.date == str(pd.to_datetime(yesterday).date())), :
                ...
                for future_time in future_times:
                    if N_POLICIES_FITTED_TUPLE == 0:  # No fitted params but at least constant ones are still taken into account
                        def model_covid_predictions(
                            ...
                            gamma_t = update_gamma_t_with_constant_params(
                                t=t, gamma_t=gamma_t, N_POLICIES_CONSTANT_TUPLE=N_POLICIES_CONSTANT_TUPLE,
                                t_start_end_constant_policy=t_start_end_constant_policy,
                                date_yesterday_int=date_yesterday_int,
                                policy_shift_names_constant=policy_shift_names_constant,
                                dict_normalized_policy_gamma_countries=dict_normalized_policy_gamma_countries,
                                params_policies_constant=params_policies_constant, t_cases=t_cases,
                                future_time=future_time,
                            )
                            # Now if we're above the future_time limit, we don't update using the constant
                            # parameters we already had but rather the ones coming from the new policy you want to be
                            # implemented. However, the fitted parameters are still being added no matter what future
                            # policy we choose to use!
                            if t > t_cases[-1] + future_time:
                                normalized_gamma_future_policy = dict_normalized_policy_gamma_countries[
                                    future_policy
                                ]
...
day_after_yesterday = "".join(str((pd.to_datetime(yesterday) + timedelta(days=1)).date()).split("-"))
...
df_global_predictions_since_100_cases_scenarios.to_csv(
    f'df_global_predictions_since_100_cases_scenarios_world_{day_after_yesterday}_CR.csv', index=False
)
