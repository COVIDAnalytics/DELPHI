...
from DELPHI_utils_ventilator_backtest import (
...
for i in [14]:
    yesterday = "".join(str(datetime.now().date() - timedelta(days=45 - i)).split("-"))
    yesterday_modified = "".join(str(datetime.now().date() - timedelta(days=45 - i -  2)).split("-"))
    ...
    try:
        pastparameters = pd.read_csv(
            PATH_TO_FOLDER_DANGER_MAP + f"predicted/Parameters_Global_{yesterday_modified}.csv"
        )
    except:
        pastparameters = None
    ...
    for continent, country, province in zip(
        ...
        if os.path.exists(PATH_TO_FOLDER_DANGER_MAP + f"processed/Global/Cases_{country_sub}_{province_sub}.csv") and country == "US":
            ...
            totalcases = totalcases[[
                        dtparser.parse(x) <= dtparser.parse(yesterday)
            ...
            if len(validcases) > 7:
                ...
                def model_covid(
                    ...
                    assert len(x) == 18, f"Too many input variables, got {len(x)}, expected 16"
                    S, E, I, AR, DHR, DQR, AD, DHD, DQD, R, D, TH, DVR, DVD, DD, DT, DVC, DVDi = x
                    ...
                    dDVCdt = r_d * p_d * p_h * p_v * I 
                    dDVDidt = r_rv * DVR + r_dth * DVD
                    dDDdt = r_dth * (DHD + DQD)
                    dDTdt = r_d * p_d * I
                    return [
                        dSdt, dEdt, dIdt, dARdt, dDHRdt, dDQRdt, dADdt, dDHDdt, dDQDdt,
                        dRdt, dDdt, dTHdt, dDVRdt, dDVDdt, dDDdt, dDTdt, dDVCdt, dDVDidt
                    ]
                ...
                df_predictions_since_today_cont_country_prov, df_predictions_since_100_cont_country_prov = (
                    data_creator.create_datasets_predictions(i)
                )
    ...
    delphi_data_saver.save_all_datasets(i,save_since_100_cases=False, website=False)
