...
def solve_and_predict_area(
    ...
    """
    Parallelizable version of the fitting & solving process for DELPHI V5, this function is called with multiprocessing
    ...
    # if os.path.exists(f"/Users/saksham/Research/temp_files/Global/Cases_{country_sub}_{province_sub}.csv"):
        ...
        if len(validcases) <= validcases_threshold:
            ...
            def residuals_totalcases(params) -> float:
                ...
                alpha, days, r_s, r_dth, p_dth, r_dthdecay, k1, k2, jump, t_jump, std_normal, k3, p_d, p_h = params
            ...
            if (OPTIMIZER in ["tnc", "trust-constr"]) or (OPTIMIZER == "annealing" and output.success):
                ...
                def solve_best_params_and_predict(optimal_params):
                    # Variables Initialization for the ODE system
                    alpha, days, r_s, r_dth, p_dth, r_dthdecay, k1, k2, jump, t_jump, std_normal, k3, p_d, p_h = optimal_params
                    optimal_params = [
                        max(alpha, dict_default_reinit_parameters["alpha"]),
                        days,
                        max(r_s, dict_default_reinit_parameters["r_s"]),
                ...
                if GET_CONFIDENCE_INTERVALS:
                   df_predictions_since_today_area, df_predictions_since_100_area = (
                       data_creator.create_datasets_with_confidence_intervals(
                           ...
                           past_prediction_file=PATH_TO_FOLDER_DANGER_MAP + f"predicted/Global_V5_{past_prediction_date}.csv",
...
if __name__ == "__main__":
    ...
    logger_filename = (
            ...
            f"model_fitting/delphi_model_V5_{yesterday_logs_filename}_{OPTIMIZER}.log"
    )
    logging.basicConfig(
    ...
    if not os.path.exists(PATH_TO_DATA_SANDBOX + f"predicted/raw_predictions/Predicted_model_state_V5_{fitting_start_date}.csv"):
    ...
    df_initial_states = pd.read_csv(
        PATH_TO_DATA_SANDBOX + f"predicted/raw_predictions/Predicted_model_state_V5_{fitting_start_date}.csv"
    )
    try:
        past_parameters = pd.read_csv(
            PATH_TO_FOLDER_DANGER_MAP
            + f"predicted/Parameters_Global_V5_{yesterday}.csv"
        )
        print(PATH_TO_FOLDER_DANGER_MAP+ f"predicted/Parameters_Global_V5_{yesterday}.csv")
    except:
    ...
    logger = logging.getLogger("V5Logger")
