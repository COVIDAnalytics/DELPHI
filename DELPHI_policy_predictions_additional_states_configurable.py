...
for continent, country, province in zip(
    ...
    if os.path.exists(
        ...
        if len(validcases) > validcases_threshold_policy:
            ...
            future_policies_area = df_policy_changes[
                (df_policy_changes.Continent == continent)
                & (df_policy_changes.Country == country)
                & (df_policy_changes.Province == province)
            ][["policy_1", "policy_2", "policy_3"]].reset_index(drop=True).iloc[0, :].tolist()
            future_times_area = df_policy_changes[
                (df_policy_changes.Continent == continent)
                & (df_policy_changes.Country == country)
                & (df_policy_changes.Province == province)
            ][["time_1", "time_2", "time_3"]].reset_index(drop=True).iloc[0, :].tolist()
            ...
            def model_covid_predictions(
                ...
                if (t > t_cases[-1] + future_times_area[0]) and (t <= t_cases[-1] + future_times_area[1]):
                    future_time = future_times_area[0]
                    future_policy = future_policies_area[0]
                    gamma_t_future = (
                ...
                elif (t > t_cases[-1] + future_times_area[1]) and (t <= t_cases[-1] + future_times_area[2]):
                    future_time = future_times_area[1]
                    future_policy = future_policies_area[1]
                    gamma_t_future = (
                            (2 / np.pi) * np.arctan(-(t_cases[-1] + future_time - days) / 20 * r_s) + 1 +
                            jump * np.exp(-(t_cases[-1] + future_time - t_jump) ** 2 / (2 * std_normal ** 2))
                    )
                ...
                elif t > t_cases[-1] + future_times_area[2]:
                    future_time = future_times_area[2]
                    future_policy = future_policies_area[2]
                    gamma_t_future = (
                            (2 / np.pi) * np.arctan(-(t_cases[-1] + future_time - days) / 20 * r_s) + 1 +
                            jump * np.exp(-(t_cases[-1] + future_time - t_jump) ** 2 / (2 * std_normal ** 2))
                    )
            ...
            print(
                "Policies: ", future_policies_area, "\t Enacting Time: ", future_times_area,
                "\t Total MAPE=", mape_data, "\t MAPE on last 15 days=", mape_data_2
            )
            # print(best_params)
            # print(country + ", " + province)
            # if future_policy in [
            ...
            df_predictions_since_today_cont_country_prov, df_predictions_since_100_cont_country_prov = (
                data_creator.create_datasets_predictions_scenario(
                    policy=str(tuple(future_policies_area)),
                    time=str(tuple(future_times_area)),
...
df_global_predictions_since_100_cases_scenarios.to_csv(
    PATH_TO_DATA_SANDBOX + f'predicted/policy_scenario_predictions/df_scenarios_provinces_j&j_{day_after_yesterday}_configured.csv',
    index=False
)
