    ...
    def save_all_datasets(self, save_since_100_cases=False, website=False):
        ...
        if save_since_100_cases:
            ...
            self.df_global_predictions_since_100_cases.to_csv(
                self.PATH_TO_FOLDER_DANGER_MAP + f"/predicted/US_KIT_{today_date_str}.csv", index=False
            )
            self.df_global_parameters.to_csv(
                self.PATH_TO_FOLDER_DANGER_MAP + f"/predicted/Parameters_KIT_{today_date_str}.csv", index=False
            )        
    @staticmethod
