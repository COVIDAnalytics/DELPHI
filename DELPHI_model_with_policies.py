...
if os.path.exists(CONFIG_FILEPATHS["data_sandbox"][USER_RUNNING] + f"policy_change_tracking_world_updated_test.csv"):
    POLICY_TRACKING_ALREADY_EXISTS = True
    df_policy_change_tracking_world = pd.read_csv(
        CONFIG_FILEPATHS["data_sandbox"][USER_RUNNING] + f"policy_change_tracking_world_updated_test.csv"
    )
else:
...
for continent, country, province in zip(
    ...
    #if province not in ["Tennessee"]:
    #    continue
    #if country not in ["Central African Republic"]:
    #    continue
...
df_tracking.to_csv(CONFIG_FILEPATHS["data_sandbox"][USER_RUNNING] + f"policy_change_tracking_world_updated_test.csv", index=False)
