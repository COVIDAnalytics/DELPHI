...
parser.add_argument(
    '--param_perturbed', '-pp', type=str, required=True,
    choices=["r_i", "r_d", "r_ri", "r_rh", "r_rv", "p_v", "p_d", "p_h"],
    help="Which of the 8 fixed parameters do we want to perturb with normal noise to assess sensitivity of results",
)
parser.add_argument(
    '--yesterday', '-y', type=str, required=True,
...
PARAM_PERTURBED = str(arguments.param_perturbed)
PATH_TO_FOLDER_DANGER_MAP = CONFIG_FILEPATHS["danger_map"][USER_RUNNING]
PATH_TO_FOLDER_SENSITIVITY_ANALYSIS = CONFIG_FILEPATHS["sensitivity_analysis"][USER_RUNNING]
...
#  Creating random normal noises for fixed parameters
SEED = 7
np.random.seed(SEED)
...
normal_noise_r_i = np.random.normal(0, 0.2 * np.abs(np.log(2) / IncubeD))  # Rate of infection leaving incubation phase
normal_noise_r_d = np.random.normal(0, 0.2 * np.abs(np.log(2) / DetectD))  # Rate of detection
normal_noise_r_ri = np.random.normal(0, 0.2 * np.abs(np.log(2) / RecoverID))  # Rate of recovery not under infection
normal_noise_r_rh = np.random.normal(0, 0.2 * np.abs(np.log(2) / RecoverHD))  # Rate of recovery under hospitalization
normal_noise_r_rv = np.random.normal(0, 0.2 * np.abs(np.log(2) / VentilatedD))  # Rate of recovery under ventilation
normal_noise_p_v = np.random.normal(0, 0.2 * np.abs(p_v))
normal_noise_p_d = np.random.normal(0, 0.2 * np.abs(p_d))
normal_noise_p_h = np.random.normal(0, 0.2 * np.abs(p_h))
if PARAM_PERTURBED == "r_i":
    r_i = r_i + normal_noise_r_i
elif PARAM_PERTURBED == "r_d":
    r_d = r_d + normal_noise_r_d
elif PARAM_PERTURBED == "r_ri":
    r_ri = r_ri + normal_noise_r_ri
elif PARAM_PERTURBED == "r_rv":
    r_rv = r_rv + normal_noise_r_rv
elif PARAM_PERTURBED == "r_rh":
    r_rh = r_rh + normal_noise_r_rh
elif PARAM_PERTURBED == "p_v":
    p_v = p_v + normal_noise_p_v
elif PARAM_PERTURBED == "p_d":
    p_d = p_d + normal_noise_p_d
elif PARAM_PERTURBED == "p_h":
    p_h = p_h + normal_noise_p_h
else:
    raise ValueError(f"Value of param_perturbed is {PARAM_PERTURBED} and cannot be processed at the moment")
...
def solve_and_predict_area(
    ...
    if os.path.exists(PATH_TO_FOLDER_DANGER_MAP + f"processed/Global/Cases_{country_sub}_{province_sub}.csv"):
        ...
        if len(validcases) <= validcases_threshold:
            ...
            PopulationR = validcases.loc[0, "death_cnt"] * 5 if validcases.loc[0, "case_cnt"] - validcases.loc[
                0, "death_cnt"] > validcases.loc[0, "death_cnt"] * 5 else 0
            ...
            """
            maxT = (pd.to_datetime(yesterday_) + timedelta(days=30) - date_day_since100).days + 1
            logging.debug(f"Number of prediction days in total since 100 cases: {maxT}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CONFIG_FILEPATHS["logs"][USER_RUNNING] + "sensitivity_analysis/"):
        os.mkdir(CONFIG_FILEPATHS["logs"][USER_RUNNING] + "sensitivity_analysis/")

    logger_filename = (
            CONFIG_FILEPATHS["logs"][USER_RUNNING] +
            f"sensitivity_analysis/delphi_model_V3_{yesterday_logs_filename}_{OPTIMIZER}_param_perturbed_{PARAM_PERTURBED}_datepred_{yesterday}.log"
    )
    logging.basicConfig(
    ...
    logging.info(
        ...
        f"generation of Confidence Intervals' flag is {GET_CONFIDENCE_INTERVALS} and the parameter perturbed is " +
        f"{PARAM_PERTURBED}"
    )
    ...
    tuples_other_US = [
        x for x in list_tuples
        if ((x[1] in ["Italy", "Spain", "Brazil", "South Africa", "Japan", "Russia"]) and (x[2] == "None"))
    ]
    list_tuples = [
        x for x in list_tuples
        if x[1] in ["US"]
    ...
    logging.info(f"Areas to be fitted in this run: {list_tuples}")
    ...
    delphi_data_saver = DELPHIDataSaver(
        path_to_folder_danger_map=PATH_TO_FOLDER_SENSITIVITY_ANALYSIS,
    ...
    delphi_data_saver.save_all_datasets(optimizer=OPTIMIZER, save_since_100_cases=SAVE_SINCE100_CASES,
                                        website=SAVE_TO_WEBSITE, param_perturbed=PARAM_PERTURBED,
                                        yesterday=yesterday)
    logging.info(
