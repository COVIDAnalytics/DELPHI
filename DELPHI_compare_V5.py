...
from DELPHI_utils_V5_dynamic import DELPHIModelComparison
...
if __name__ == "__main__":
    ...
    logger_filename = (
            ...
            f"model_comparison/delphi_model_V5_{yesterday_logs_filename}.log"
    )
    logging.basicConfig(
    ...
    if RUN_MODEL:
        ...
        os.system(
            f'python3 DELPHI_model_V5.py -rc tnc-run-config.yml'
        )
        os.system(
            f'python3 DELPHI_model_V5.py -rc annealing-run-config.yml'
        )
    ...
    global_parameters_tnc = pd.read_csv(
        PATH_TO_FOLDER_DANGER_MAP + f"/predicted/Parameters_Global_V5_{today_date_str}.csv"
    )
    global_parameters_annealing = pd.read_csv(
        PATH_TO_FOLDER_DANGER_MAP + f"/predicted/Parameters_Global_V5_annealing_{today_date_str}.csv"
    )
    ...
    global_annealing_predictions_since_100days = pd.read_csv(
        PATH_TO_FOLDER_DANGER_MAP + f'predicted/Global_V5_annealing_since100_{today_date_str}.csv'
    )
    total_tnc_predictions_since_100days = pd.read_csv(
        PATH_TO_FOLDER_DANGER_MAP + f'predicted/Global_V5_since100_{today_date_str}.csv'
    )
