...
from DELPHI_utils_V3_static import DELPHIBacktestSensitivity
...
logger_filename = (
        ...
        f"backtest_sensitivity_analysis/{logger_filename_date}_delphi_backtest_prediction_date" +
        f"_{PREDICTION_DATE}_n_days_{N_DAYS_BACKTEST}_sensitivity_analysis_param_pert_{PARAM_PERTURBED}.log"
)
logging.basicConfig(
...
if __name__ == "__main__":
    ...
    df_backtest_metrics["param_perturbed"] = PARAM_PERTURBED
    df_backtest_metrics.to_csv(
        PATH_TO_DATA_SANDBOX + f"backtest_outputs_sensitivity_analysis/backtest_pd_{PREDICTION_DATE}_n_days_{N_DAYS_BACKTEST}_param_perturbed_{PARAM_PERTURBED}.csv", index=False
    )
