...
from DELPHI_utils_CT_static import (
...
from DELPHI_utils_CT_dynamic import get_bounds_params_from_pastparams
from DELPHI_params_CT import (
    fitting_start_date,
...
def solve_and_predict_area(
    ...
    if os.path.exists(PATH_TO_FOLDER_DANGER_MAP + f"processed/Global/Cases_{country_sub}_{province_sub}.csv"):
        ...
        if len(validcases) <= validcases_threshold:
            ...
            def model_covid(
                t, x, alpha, days, r_s, r_dth, p_dth, r_dthdecay, k1, k2, jump, t_jump, std_normal, k3, p_h, t_rh
            ) -> list:
                """
            ...
            def residuals_totalcases(params) -> float:
                ...
                alpha, days, r_s, r_dth, p_dth, r_dthdecay, k1, k2, jump, t_jump, std_normal, k3, p_h, t_rh = params
                ...
                params = (
                    ...
                    max(p_h, dict_default_reinit_lower_bounds["p_h"]),
                    max(t_rh, dict_default_reinit_lower_bounds["t_rh"])
                )
...
if __name__ == "__main__":
    ...
    try:
        past_parameters = pd.read_csv(
            ...
            + f"predicted/Parameters_Global_CT_{yesterday}.csv"
        )
    except:
