...
import json
from DELPHI_params_KIT import (TIME_DICT, MAPPING_STATE_CODE_TO_STATE_NAME, default_policy,
...
class DELPHIDataSaver:
    ...
    def save_all_datasets(self, save_since_100_cases=False, website=False):
        ...
        self.df_global_parameters.to_csv(
            self.PATH_TO_FOLDER_DANGER_MAP + f"/predicted/Parameters_DE_KIT_{today_date_str}.csv", index=False
        )
        # Save predictions since today
        self.df_global_predictions_since_100_cases.to_csv(
            self.PATH_TO_FOLDER_DANGER_MAP + f"/predicted/DE_KIT_{today_date_str}.csv", index=False
        )
    @staticmethod
