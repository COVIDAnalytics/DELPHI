    ...
    if (
        ...
        if len(validcases) > validcases_threshold:
            ...
            print("Policy Shifts Fitted:", N_POLICIES_FITTED_TUPLE, " ##### "
                                                                    "Policy Shifts Constant:",
                  N_POLICIES_CONSTANT_TUPLE)
            ...
            def residuals_totalcases(params):
                ...
                if len(params) == n_params_without_policy_params:
                    ...
                    params = max(alpha, 0), days, max(r_s, 0), max(r_dth, 0), max(min(p_dth, 1), 0), max(k1, 0), max(k2,
                                                                                                                     0)
                elif len(params) == 8:
...
if __name__ == "__main__":
    for n_days_before in range(max_days_before, 0, -1):
        ...
        if os.path.exists(PATH_TO_DATA_SANDBOX + f"policy_change_tracking_world_updated_final_multiproc_0612.csv"):
            POLICY_TRACKING_ALREADY_EXISTS = True
            df_policy_change_tracking_world = pd.read_csv(
                PATH_TO_DATA_SANDBOX + f"policy_change_tracking_world_updated_final_multiproc_0612.csv"
            )
        else:
